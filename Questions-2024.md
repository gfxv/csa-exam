...
RISC
- архитектура с простым кодированием упрощенного набора команд 
- увеличивает быстродействие

**Обрубим все до минимума и оптимизируем его, разложим по полочкам. Теперь и аспирант напишет**

Предпосылки
- Сложные операции (встречаются редко, заменимы группами команд и процедурами)
- Появились ЯП высокого уровня (компилятор сам оптимизирует)
- Одинаковая длинна инструкции
	- => простота декодера
- Место из-под микрокоманд можно закинуть в кэш
- Легче оптимизировать меньшее число команд
- Параллелизм уровня инструкций (разбиваем обработку инструкции на несколько этапов и выполняем параллельно для разных команд) 

**Pipeline**  
![](http://csa.edu.swampbuds.me/fig/proc-risc-pipeline-vs-non-pipeline.jpg)

Выделяем стадии, +- равны, за 1 такт.

**Наблюдается постепенный переход с CISC НА RISC.**

Мобильные — уже.  
ПК и сервера — в процессе. apple вперед!  
РИСК долго выходил на рынок из-за меньшей серийности.  
Риск ядро внутри Циск - через транспайлер (intel)

### 15. Конвейеризированное исполнение команд. Стадии конвейера. Виды конфликтов (по данным, по управлению), их примеры и влияние на производительность. Достоинства и недостатки.

- Используется в RISC
- основано на разбивке обработки инструкции на несколько этапов
- выполнении параллельно для разных команд, 1 такт - 1 стадия конвейера

Стадии конвейера:
- выделить стадии выполнения команд 
	- (стадии конвейера эквивалентны по времени)
- организовать внутренние структуры процессора так, чтобы
    ...
    - сегменты связаны регистрами (через них передача данных), комбинационные схемы сбалансированы;
    - все части процессора управляются одним тактовым сигналом;
- загружать в процессор команды каждый такт;
- получать результаты выполнения команд каждый такт;
- разрешать конфликты параллельно выполняемых команд
	- (возможность взаимодействия инструкций, обращения к одному участку памяти).

Типовые стадии:
- Instruction Fetch. Чтение инструкции по счётчику команд.
- Instruction Decode. Декодировать инструкцию и считать регистры.
- Instruction Execute. Операций изменения данных.
- Memory Access. Чтение/запись операндов из памяти/в память.
- Write Back. Запись результата в регистры.

Проблемы:
- Доступ к одному и тому же ресурсу памяти в 1 такт 
	- Решение
		- гарвардская архитектура
		- двухпортовая память
		- использование пустых инструкций для конфликтных мест (но снижает эффективность)
- Конфликты по данным:
    - Read After Write - запаздывание по записи результата относительно его выборки → **чтение старых данных.** 
	    - когда команда пытается прочитать операнд, который был изменен другой командой, но еще не записан в память.
	    - Решение
		    - исполнение не по порядку (компилятор сует между смежными операциями другие)
		    - переименование регистров на уровне компилятора
		    - пузырек
		    - проброс операндов (проброс результата на нужную стадию, только вперед, поэтому либо напрямую, либо используя пузырек)
    - Write After Read - чтение данных для операции уже после перезаписи (приколы компилятора с перемешиванием данных) → неверная арифм. операция
	    - когда команда пытается записать операнд в память или регистр, который был прочитан предыдущей командой, но еще не записан.
	- Write After Write - 2 записи подряд в одно место → смешение их при выполнении
		- когда две команды пытаются записать результат в один и тот же операнд (регистр или ячейку памяти), но в неправильном порядке
	- Read After Read - 2 чтения подряд из 1 регистра (не проблема)
- Конфликт по управлению
	- В конвейере команда, которая не должна исполняться (из-за перехода)
	- Решение:
		- Пузырек
		- Сброс конвейера
		- Минимизаций условных переходов:
			- loop unrolling - удаление или изменение циклов таким образом, чтобы уменьшить их или число их итераций. Увеличивает скорость за счет удаление инструкций управления циклом и тестирования
	    - Предсказания переходов, статистическое и динамическое (угадывание данных и их выборки). `Пример - переход назад (из цикла) произойдет почти всегда.`
...
- *Read After Write* - запаздывание по записи результата относительно его выборки → **чтение старых данных.** 
	...
	- Решение
		...
		- пузырек
		- проброс операндов (проброс результата на нужную стадию, только вперед, поэтому либо напрямую, либо используя пузырек)
- *Write After Rea*d - чтение данных для операции уже после перезаписи (приколы компилятора с перемешиванием данных) → неверная арифм. операция
	- когда команда пытается записать операнд в память или регистр, который был прочитан предыдущей командой, но еще не записан.
- *Write After Write* - 2 записи подряд в одно место → смешение их при выполнении
	- когда две команды пытаются записать результат в один и тот же операнд (регистр или ячейку памяти), но в неправильном порядке
- *Read After Read* - 2 чтения подряд из 1 регистра (не проблема)
...
- Решение:
	...
	- Минимизаций условных переходов:
		- loop unrolling - удаление или изменение циклов таким образом, чтобы уменьшить их или число их итераций. Увеличивает скорость за счет удаление инструкций управления циклом и тестирования
	- Предсказания переходов, статистическое и динамическое (угадывание данных и их выборки). `Пример - переход назад (из цикла) произойдет почти всегда.`
		- Статика - Использует статистику или информацию о коде для принятия решения (заложенный паттерн)
		- Динамика - на истории выполнения инструкций (буфер предсказания переходов, накапливаем опыт переходов)

Конвейер:
- Плюсы
    - Общее повышение производительности
- Недостатки
    - Замедление исполнения отдельной команды
    - Не все операции выполняются за 1 машинный цикл
    - Конфликты
    - Непредсказуемое время исполнения

### 17. Что такое SOP (Stack-Oriented Processors, стековый процессор)? Роль в развитии компьютерных систем. Применение на практике. Достоинства и недостатки. Отличия от архитектуры фон Неймана. Особенности программирования.

ROSC - стековый процессор
- Нет понятия операнда (кроме взаимодействия со стеком)
- только АЛУ и стек
- Обработка данных - на стеке
- Условные и безусловные переходы - через стек
- Обычно несколько стеков
- Достоинства:
    - Высокоуровневая архитектура 
	    - высокоуровненвый код
	    - поддержка процедур
	    - автоматическая память
	    - рекурсия
	    - простой компилятор
    - Простая система команд
    - Просто сменить контекст 
	    - переключаемся на другой стек и всё
    - Высокая производительность
    - Кэш и потоки - проще реализация
- Недостатки:
    - Низкая эффективность при большом размере стека
    - Не можем располагать сложные структуры на стеке
    - Нет параллелизма инструкций (Конвейера) (каждые инструкции завязаны друг на друге)
    - Фундаментально отличается от других архитектур

// более энергоэффективны (по сравнению с тем же RISC)
// => лучше для каких-нибудь встроенных систем (???)

### 18. Поддержка операций ввода-вывода в фон Неймановских процессорах. Поддержка на уровне системы команд (порт, отображение в память). Программно-управляемый ввод-вывод. Ввод-вывод через механизм прерываний. Механизм прямого доступа к памяти.

// Интерфейс устройства для CPU
У ВУ должны быть (для общения с CPU):
- регистры данных
- регистры состояния
- *протокол взаимодействия*

Пример взаимодействие по шагам:
- Конфигурация устройства 
	- (скорость передачи, настройки протокола)
- Проверка состояния 
	- (конфигурация, ее функционирование)
- Отправка данных
- Проверка статуса ответа
- Чтение ответа

Варианты ввода-вывода:
- `Программно-управляемый ` - реализует процессор, все через его инструкции (в самых примитивных системах)
	- все операции реализуются процессором
		- процессор выполняет все действия, связанные с вводом-выводом, под своим полным контролем
	- постоянное наблюдение за готовностью ВУ (polling)
	- `Теорема Котельникова`: частота принимающего сигнал должна быть минимум вдвое больше отправителя
	- IO может быть продолжительный, т.к. проц ожидает готовности устройства
- `По прерыванию` - использовать прерывание или иное внешнее событие, чтобы переключиться на ввод-вывод
	- снимает с проца задачу постоянного наблюдения
	- ВУ управляют процессом ввода-вывода, генерируя сигналы прерывания.
	- IO может быть более быстрой, т.к. процессор обрабатывает сигнал прерывания и выполняет соответствующую программу
- `Прямой доступ к памяти` - через контроллер пдп, делает всё сам
	- Процессор может работать на другой частоте
	- скорость и эффективность повышаются
	- Проблемы синхронизации доступа к MEM:
		- пакетный режим - приоритет DMA
			- `burst mode` -  передача единой операцией, не может быть прервана процом
		- прозрачный режим - приоритет CPU
			- только когда проц не взаимодействует с памятью
		- циклический - приоритет меняется

*// IO Channel это старье, что вытеснили DMA*

С точки зрения системы команд (ISA):
- `Port`
	- IO через спец. инструкции (in/out)
	- адресация регистров ВУ не зависит от адресации памяти
	- +
		- Минимизация логики управления (малое адресное пространство). Оптимизация IO
		- адресное пространство памяти однородное
	- -
		- усложнение ISA и процессора.
		- ограничено адресное пространство IO
		- дополнительное ("лишнее") копирования данных.
- `Mem`
	- регистры ВУ отображаются в память
	- IO через инструкции доступа в память
	- можно реализовать без изменения микроархитектуры проца
	- +
		- простота процессора
		- обработка данных без переноса в память
		- большое адресное пространство (кол-во устройств не ограничено устройством процессора)
	- -
		- одна шина для ввода-вывода и доступа к памяти
		- Неоднородность памяти
		- *конфликты в кэшами и параллелизм уровня инструкций*

SPI (Serail Peripheral Interaface) 
- последовательный интерфейс передачи данных
- обеспечивает синхронную передачу между микроконтроллером и ВУ
- `MOSI` — отдаем данные в устройство
	- выход ведущего, вход ведомого (master out slave in)
- `MISO` (Master In Slave Out) — получаем данные из устройства 
	- вход ведущего, выход ведомого (master in slave out)
- `SCLK`  — последовательный тактовый сигнал
	- передача такт. сигнала для ведомых устройств.
	- чтобы понять когда шлем данные (serial clock)
- `CS` (Chip Select) — выбор чипа или устройства


### 19. Параллелизм уровня задач. Кооперативная многозадачность. Принцип работы и подходы к реализации. Примеры использования. Достоинства и ограничения. Зелёные процессы. Проблема синхронизации процессов по управлению и по данным. Сравнение с альтернативами.

Используем параллелизм когда нужно
- работать сразу над несколькими задачами
- повысить уровень использования ресурсов 
	- (не простаиваем, а занимаемся чем-то полезным).
- повысить производительность компьютера 
	- (больше дел за единицу времени).

Параллелизм уровня задач
- Параллельное выполнение нескольких программ

Проблемы:
- Нейман не рассчитан под параллелизм
	- тк есть PC, который переходит либо дальше, либо jmp
- Поток инструкций один

`Кооперативная многозадачность` (соответствует фон нейману)
- сами процессы знают, что будут прерваны в нужных местах
- Текущая задача забирает все процессорное время
- Фоновые - заморозка
- Доступ ко всем необходимым ресурсам

Механизм
- `Остановка` - добровольная передача управления диспетчеру 
	- (`hlt/switch/pause`)
- `Cохранение` состояния (регистры, память, ввод-вывод, кеш...)
- `Планирование` - кто получит процессор
- `Возобновление` остановленного процессора 
	- восстановление состояния и передача управления
- `Изоляция` задачи - независимое выполнение, безопасность
- `Взаимодействие` между задачами - передача данных и сигналов, общие ресурсы

Подходы к реализации:
- Имитация через конечные автоматы
	- в примитивных встроенных системах
- Диспечтер 
	- на уровне ОС (сисколы hlt/pause...)
		- современные больше на вытисняющую
	- в рантайме или VM (шитый код)
	- программный код (asyncio, even-loop, callbacks)

// шитый код - когда в перемешку идёт байт-код ВМ и машинный код

Green Threads
- это потоки выполнения
- управление которыми производит VM или рантайм либа (вместо ОС)
- эмулируют многопок, не опираясь на возможности ОС
- *позволяет им работать в средах, не имеющих встроенной поддержки потоков*

// ключевые места перекл горутины - вызов функции или циклы

Проблемы синхронизации 
По управлению: 
- deadlock - каждый процесс удерживает ресурс, необходимый другому процессу, и они не могут продолжать свою работу
По данным:
- Гонки (?) - когда несколько процессов пытаются одновременно получить доступ к общим данным и изменить их


Преимущества:
- нет гонок, транзакций (т.к. знаем, что процесс не будет внезапно прерван)
- контроль за ресурсами со стороны задачи
- известны точки остановки

Недостатки:
- Заметить бесконечный loop
- Сбой задачи - глобальный сбой
	- если зависнет одна, зависнет всё
- Низкая эффективность ввода-вывода
- Сложность разработки интерактивных приложений (очередь задач)
	- если что-то произошло, мы всё равно ждём пока дадут процесс
...
`Вытисняющая многозадачность` (истинная многозадачность)
- Планировщик
- Фоновые - заморозка
- Мгновенная реакция на действия пользователя

Механизм
- `Прерывания` - забрать процессор независимо от желания
- `Cохранение` состояния (регистры, память, ввод-вывод, кеш...)
- `Планирование` - кто получит процессор
- `Возобновление` остановленного процессора 
	- восстановление состояния и передача управления
...
- Прерывания
	...
	- Вызов обработчика прерывания (ISR)
	- Завершение ISR.
...
Преимущества:
- возможна "мгновенная" реакция на действия
- контроль за ресурсами со стороны ОС

Недостатки:
- Непредсказуемая длительность работы. 
	- Реальное время - мы без понятия, сколько раз нашу программу прервут
- Непредсказуемость переключения (гонки, deadlock, синхронизация)

Проблемы синхронизации:
- MapReduce - запускаем алгоритм на разных компонентах нашей системы и на одном из них происходит ошибка - возникает сложность остановки остальных процессов
...
Виды прерываний 
- Аппаратные
	- Внешние (асинхронные для внутренних циклов процессора)
		- переполнение таймера, нажатие клавиш, сетевой пакет
	- Внутренние (синхронные)
		- деление на 0, ошибка доступа к памяти и т.п.
	- *`Асинхронно - в любой момент может произойти (тут имеются в виду такты)`*
	- *`Синхронно - должны быть готовы обработать прерывание в определенное время (такт)`*
- Программные (вызывается инструкцией)
	- взаимодействие программы и ОС (опять про системные вызовы)

Для ВУ - `контроллер прерываний`:
- Обычно реализуется аппаратно (скорость и параллелизм)
- Конкурирующие прервания (приоритеты, потери, очереди):
    - `Маскируемые/Немаскируемые` 
	    - (какие устройства лучше приглушить, а какие почаще слушать)
    - `Относительные/Абсолютные` 
	    - (прерывает прерывание)
	    - если прерывание во время прерывания 
	    - `относительное` - игнорируем
	    - `абсолютные` - обрабатываем
- Прерывания могут реагировать на различные события:
    - По фронту
    - По уровню
    - По сообщению (содержит дополнительные данные)
    - По дверному звонку

// Сигнал и данные разделены

`Сторожевой таймер` — аппарат. схема контроля от зависания системы  
(_аппаратно_ реализованная схема контроля от зависания системы)
